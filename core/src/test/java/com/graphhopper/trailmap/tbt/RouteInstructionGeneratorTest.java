...
""";
    ...
    // =====================================================================================
    // DIAGNOSTIC: HTTP 500 "Edge X is not connected to node Y" on a Joensuu trailmap_foot loop.
    // The disconnected edge chain is fed to buildSyntheticPath().walkEdge(). This test reproduces
    // the exact API payload, then A/Bs the SUPPRESS_BOUNDARY_UTURN flag to isolate whether the
    // merge path (generateWithBoundaryUturnSuppression / mergeSections) or the legacy stitch path
    // is producing the broken chain.
    // =====================================================================================
    @Test
    void diagnoseJoensuu500_disconnectedEdgeChain() throws Exception {
        org.junit.jupiter.api.Assumptions.assumeTrue(hopper != null, "graph cache required");

        ObjectMapper mapper = new ObjectMapper();

        boolean savedFlag = RouteInstructionGenerator.SUPPRESS_BOUNDARY_UTURN;
        try {
            for (boolean suppress : new boolean[]{true, false}) {
                RouteInstructionGenerator.SUPPRESS_BOUNDARY_UTURN = suppress;
                TrailmapInstructionRequest request =
                        mapper.readValue(JOENSUU_500_JSON, TrailmapInstructionRequest.class);
                RouteInstructionGenerator generator = new RouteInstructionGenerator(
                        hopper, hopper.getBaseGraph(), hopper.getEncodingManager(), hopper.getTranslationMap());
                System.out.printf("%n=== SUPPRESS_BOUNDARY_UTURN=%s ===%n", suppress);
                try {
                    RouteInstructionGenerator.Result result = generator.generate(request);
                    System.out.printf("  OK: %d instructions, %d polyline pts%n",
            ...
            RouteInstructionGenerator.SUPPRESS_BOUNDARY_UTURN = savedFlag;
        }
    }

    // Per-segment connectivity probe: replicate generate()'s routing loop (heading + start-continuity),
    // route each routable segment, and check whether the RAW edge_id chain and the dedup'd chain
    // (extractEdgeIds via reflection) are node-connected. Pinpoints which segment yields the break.
    @Test
    void diagnoseJoensuu500_perSegmentEdgeChains() throws Exception {
        org.junit.jupiter.api.Assumptions.assumeTrue(hopper != null, "graph cache required");

        ObjectMapper mapper = new ObjectMapper();
        TrailmapInstructionRequest request = mapper.readValue(JOENSUU_500_JSON, TrailmapInstructionRequest.class);
        ...
        for (RouteInstructionGenerator.Chunk chunk : chunks) {
            ...
            if (section == null) {
                System.out.printf("chunk %d: NON-ROUTABLE gap -> reset start override%n", chunkIdx);
            ...
            for (TrailmapInstructionRequest.Coordinates c : section.points)
                req.addPoint(new GHPoint(c.getLat(), c.getLng()));
            ...
            GHResponse rsp = hopper.route(req);
            if (rsp.hasErrors()) {
                System.out.printf("chunk %d: ROUTING ERROR %s%n", chunkIdx, rsp.getErrors());
                continue;
            }
            ResponsePath path = rsp.getBest();
            List<PathDetail> dets = path.getPathDetails().get("edge_id");
            List<Integer> raw = new java.util.ArrayList<>();
            for (PathDetail d : dets) raw.add((Integer) d.getValue());

            @SuppressWarnings("unchecked")
            List<Integer> dedup = (List<Integer>) extractEdgeIds.invoke(generator, path);

            // ORACLE: GH's oriented edge_key gives the authoritative travel direction. The first
            // oriented edge's baseNode IS the true fromNode — unambiguous even for parallel/out-and-back.
            int oracleFromNode = -1;
            List<PathDetail> keyDets = path.getPathDetails().get("edge_key");
            if (keyDets != null && !keyDets.isEmpty()) {
                int firstKey = ((Number) keyDets.get(0).getValue()).intValue();
                oracleFromNode = baseGraph.getEdgeIteratorStateForKey(firstKey).getBaseNode();
            }

            // Reproduce production buildSyntheticPath: resolveFromNode (incl. proximity fallback) + walk.
            TrailmapInstructionRequest.Coordinates startCoord = section.points.get(0);
            String prodBreak = null;
            int prodFromNode = -1;
            if (!dedup.isEmpty()) {
                prodFromNode = (int) resolveFromNode.invoke(generator, dedup, startCoord);
                prodBreak = walkFromBreak(baseGraph, dedup, prodFromNode);
            }
            String oracleBreak = oracleFromNode >= 0 ? walkFromBreak(baseGraph, dedup, oracleFromNode) : "no-key";

            boolean disagree = prodFromNode != oracleFromNode;
            if (prodBreak != null || disagree) {
                System.out.printf("chunk %d: heading=%s edges=%d prodFromNode=%d oracleFromNode=%d disagree=%s prodBreak=[%s] oracleWalk=[%s]%n",
                        chunkIdx, heading == null ? "free" : String.format("%.0f", heading),
                        dedup.size(), prodFromNode, oracleFromNode, disagree, prodBreak, oracleBreak);
            }

            PointList pts = path.getPoints();
            nextStartOverride = pts.size() >= 1
                    ? coordOf2(pts.getLat(pts.size() - 1), pts.getLon(pts.size() - 1)) : null;
        }
    }

    private static TrailmapInstructionRequest.Coordinates coordOf2(double lat, double lng) {
        TrailmapInstructionRequest.Coordinates c = new TrailmapInstructionRequest.Coordinates();
        c.setLat(lat);
        c.setLng(lng);
        return c;
    ...
    /** Walk the chain from a GIVEN start node (mirrors production buildSyntheticPath). */
    private static String walkFromBreak(BaseGraph g, List<Integer> edgeIds, int fromNode) {
        int cur = fromNode;
        for (int i = 0; i < edgeIds.size(); i++) {
            EdgeIteratorState e = g.getEdgeIteratorState(edgeIds.get(i), Integer.MIN_VALUE);
            if (e.getBaseNode() == cur) cur = e.getAdjNode();
            else if (e.getAdjNode() == cur) cur = e.getBaseNode();
            else return "idx " + i + " edge=" + edgeIds.get(i) + " endpoints(" + e.getBaseNode() + ","
                    + e.getAdjNode() + ") not connected to node " + cur;
        }
        return null;
    }

    // Failing Joensuu loop payload (HTTP 500: "Edge 200795613 is not connected to node 158011608").
